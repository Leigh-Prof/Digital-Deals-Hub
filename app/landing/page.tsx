...
import {
  ...
  Download, TrendingUp, Users, ShieldCheck, Clock, Smartphone,
  BookOpen, Layers, Palette, Brain, BarChart2, FileText,
  Video, PenTool, Briefcase, GraduationCap, Baby
} from 'lucide-react'
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <span className="inline-block px-4 py-1.5 rounded-full bg-blue-50 text-blue-700 text-xs font-semibold tracking-widest uppercase mb-4 border border-blue-100">
      {children}
    </span>
  )
}
...
export default function LandingPage() {
  ...
  return (
    <div className="bg-white text-slate-900 overflow-x-hidden">
      ...
      {/* ── Sticky mobile CTA ──────────────────────────────────────────── */}
      <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-white/95 backdrop-blur border-t border-slate-200 px-4 py-3 flex items-center justify-between gap-3 shadow-2xl">
        <div>
          <p className="text-xs text-slate-500 leading-none">Lifetime Access</p>
          <p className="text-lg font-bold text-slate-900">₱699 <span className="line-through text-slate-400 text-sm font-normal">₱4,980</span></p>
        </div>
      ...
      <AnimatePresence>
        {showTop && (
          <motion.button
            ...
            className="fixed bottom-24 md:bottom-8 right-4 z-50 w-11 h-11 rounded-full shadow-lg bg-white border border-slate-200 flex items-center justify-center hover:bg-slate-50 transition-colors"
          >
      ...
      {/* ── Messenger float ────────────────────────────────────────────── */}
      <a
        href="https://m.me/digitaldealshubredirect"
        target="_blank"
      ...
      <section className="relative min-h-screen flex flex-col" style={{ background: 'linear-gradient(160deg,#0f172a 0%,#1e3a8a 50%,#1e1b4b 100%)' }}>
        ...
        <motion.div
          ...
          className="relative z-10 mx-auto mb-10 flex items-center gap-6 px-8 py-5 rounded-3xl bg-white/10 backdrop-blur border border-white/15 shadow-xl"
        >
      ...
      <section className="py-24 px-5 bg-slate-50">
        <div className="max-w-3xl mx-auto text-center">
          <FadeUp>
            ...
            <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-900 mb-6" style={{ fontFamily: 'Manrope, sans-serif' }}>
              We Did the Hard Work<br />So You Don&apos;t Have To
            </h2>
          </FadeUp>
          <FadeUp delay={0.1}>
            <div className="prose prose-slate prose-lg mx-auto text-slate-600 leading-relaxed text-left space-y-4">
              ...
              <p>We spent <strong className="text-slate-800">thousands of pesos</strong> on trial and error. Some products were amazing. Some were a complete waste of money. Over time, we learned to tell the difference.</p>
              ...
              <p>Now, instead of making you go through the same expensive journey, <strong className="text-slate-800">we&apos;ve already done the hard work for you.</strong></p>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {PROBLEMS.map((p, i) => (
              <FadeUp key={p} delay={i * 0.08}>
                <div className="flex items-start gap-4 p-5 rounded-2xl border border-red-100 bg-red-50">
      ...
      ══════════════════════════════════════════════════════════════════ */}
      <section id="whats-inside" className="py-24 px-5 bg-white">
              ...
              <FadeUp key={label} delay={i * 0.06}>
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <span className="inline-block px-3 py-1 rounded-full bg-slate-100 text-slate-600 text-xs font-semibold">Worth {value}</span>
                  </div>
                </motion.div>
              </FadeUp>
            ))}
      ...
      <section className="py-24 px-5 bg-slate-50">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-8 mb-16">
            <FadeUp delay={0.1}>
              <div className="rounded-3xl p-8 bg-white border border-slate-100 shadow-sm h-full">
                <div className="w-12 h-12 rounded-2xl bg-blue-100 flex items-center justify-center mb-5">
                  <Briefcase size={24} className="text-blue-600" />
                </div>
                <h3 className="text-xl font-bold text-slate-900 mb-3" style={{ fontFamily: 'Manrope, sans-serif' }}>Option 1 — Use It For Your Business</h3>
            ...
            <FadeUp delay={0.2}>
              <div className="rounded-3xl p-8 bg-white border border-slate-100 shadow-sm h-full">
                ...
                <h3 className="text-xl font-bold text-slate-900 mb-3" style={{ fontFamily: 'Manrope, sans-serif' }}>Option 2 — Build Your Own Digital Business</h3>
                <ul className="space-y-3 text-slate-600">
              ...
              {['Download', 'Learn', 'Create', 'Market', 'Earn'].map((step, i, arr) => (
                <div key={step} className="flex items-center gap-2 sm:gap-4">
                  <div className="flex flex-col items-center">
                    ...
                    <span className="text-xs font-semibold text-slate-600 mt-2">{step}</span>
                  </div>
                  {i < arr.length - 1 && <span className="text-slate-300 text-2xl mb-4">→</span>}
                </div>
              ))}
            </div>
          </FadeUp>
      ...
      <section className="py-24 px-5 bg-white">
        <div className="max-w-4xl mx-auto">
          ...
          <FadeUp delay={0.05} className="text-center mb-12">
            <p className="text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-xl px-5 py-3 inline-block max-w-2xl">
          ...
          <div className="grid sm:grid-cols-3 gap-5 mb-10">
            {[
              ...
              <FadeUp key={label} delay={i * 0.1}>
                <div className="rounded-2xl p-6 border border-slate-100 bg-slate-50 text-center">
                  <p className="text-xs text-slate-400 uppercase tracking-wide mb-2">{label}</p>
                  <p className="text-slate-500 text-sm mb-1">{calc}</p>
                  <p className="text-2xl font-extrabold text-slate-900" style={{ fontFamily: 'Manrope, sans-serif' }}>{result}</p>
          ...
          <FadeUp delay={0.4} className="rounded-2xl p-6 bg-blue-50 border border-blue-100 text-center">
      ...
      <section className="py-24 px-5 bg-slate-50">
        <div className="max-w-6xl mx-auto">
          ...
          <FadeUp delay={0.05} className="text-center mb-12">
            <p className="text-xs text-slate-400 italic">*Sample/demo testimonials for illustration purposes.</p>
          </FadeUp>

          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {TESTIMONIALS.map(({ name, role, stars, text }, i) => (
              <FadeUp key={name} delay={i * 0.07}>
                <motion.div whileHover={{ y: -4 }} className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm">
                  <StarRow count={stars} />
                  <p className="text-slate-600 text-sm leading-relaxed mt-3 mb-5">&ldquo;{text}&rdquo;</p>
